...
public class Query12 extends HttpServlet {
    ...
    throws ServletException, IOException {
        try {
            ...
            List<IRaceInfo> allRaces = RaceInfoManager.getAllRaces();
            request.setAttribute("allRaces", allRaces);

            if (request.getParameterMap().containsKey("selectedRace"))
            {
                int selectedRace = Integer.valueOf(request.getParameter("selectedRace"));
                int selectedDept = Integer.valueOf(request.getParameter("selectedDept"));
                final Dictionary<String, Integer> results =
                        UsageHistoryManager.GetCmdCountByRaceAndDept(selectedRace, selectedDept);
            ...
            RequestDispatcher view = request.getRequestDispatcher("12.jsp");
